...
** This will be quite complicated, as want to post results to file, but as exposures differ extracting the results will be variable specific. To adjust for multiple corrections will use conservative bonferroni adjustment when constructing confidence intervals and interpreting p-values - As 34 exposures, will a Bonferroni p-value threshold of 0.05/34 = 0.0015.
...
foreach var of varlist ageAt28 nonWhiteEthnic maritalStatus mobility rural parity education maternalEdu highSocClass income IMD townsendDep housing financeDiffs chLifeEvents_wgt chLifeEvents_total crowding neighPercept partnerAbsence logicMemory-selfEsteem {
	...
	// Next, how we run the analysis will depend on the type of variable - So need to specify whether variable is continuous/binary (as these can be treated the same), or categorical. Will start with cont/binary variables - Although need to analyse 'age' separately first as will be adjusted for in all other models
	if "`var'" == "ageAt28" {
		mlogit Y3153_cat `var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3153_cat if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3153_cat `var', baseoutcome(1) rrr
		est store main
	...
	// Next, analyse the rest of the continuous/binary variables
		...
		mlogit Y3153_cat ageAt28 `var', baseoutcome(1) rrr
		...
		// Now for interaction model
		mlogit Y3153_cat c.ageAt28##c.`var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3153_cat ageAt28 if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3153_cat ageAt28 `var', baseoutcome(1) rrr
		est store main
	...
	// Next, go through the remaining categorical variables and code as needed - In all cases will treat lowest category as reference
		...
		if `cats' == 2 {
			...
			mlogit Y3153_cat ageAt28 i.`var', baseoutcome(1) rrr
			...
			// Now for interaction model
			mlogit Y3153_cat c.ageAt28##i.`var', baseoutcome(1) rrr
...
foreach var of varlist ageAt28 nonWhiteEthnic maritalStatus mobility rural parity education maternalEdu highSocClass income IMD townsendDep housing financeDiffs chLifeEvents_wgt chLifeEvents_total crowding neighPercept partnerAbsence logicMemory-selfEsteem {
	...
	// Next, how we run the analysis will depend on the type of variable - So need to specify whether variable is continuous/binary (as these can be treated the same), or categorical. Will start with cont/binary variables - Although need to analyse 'age' separately first as will be adjusted for in all other models
	if "`var'" == "ageAt28" {
		mlogit Y3160_new `var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3160_new if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3160_new `var', baseoutcome(1) rrr
		est store main
	...
	// Next, analyse the rest of the continuous/binary variables
		...
		mlogit Y3160_new ageAt28 `var', baseoutcome(1) rrr
		...
		// Now for interaction model
		mlogit Y3160_new c.ageAt28##c.`var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3160_new ageAt28 if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3160_new ageAt28 `var', baseoutcome(1) rrr
		est store main
	...
	// Next, go through the remaining categorical variables and code as needed - In all cases will treat lowest category as reference
		...
		if `cats' == 2 {
			...
			mlogit Y3160_new ageAt28 i.`var', baseoutcome(1) rrr
			...
			// Now for interaction model
			mlogit Y3160_new c.ageAt28##i.`var', baseoutcome(1) rrr
...
foreach var of varlist ageAt28 nonWhiteEthnic maritalStatus mobility rural parity education maternalEdu highSocClass income IMD townsendDep housing financeDiffs chLifeEvents_wgt chLifeEvents_total crowding neighPercept partnerAbsence logicMemory-selfEsteem {
	...
	// Next, how we run the analysis will depend on the type of variable - So need to specify whether variable is continuous/binary (as these can be treated the same), or categorical. Will start with cont/binary variables - Although need to analyse 'age' separately first as will be adjusted for in all other models
	if "`var'" == "ageAt28" {
		mlogit Y3170_new `var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3170_new if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3170_new `var', baseoutcome(1) rrr
		est store main
	...
	// Next, analyse the rest of the continuous/binary variables
		...
		mlogit Y3170_new ageAt28 `var', baseoutcome(1) rrr
		...
		// Now for interaction model
		mlogit Y3170_new c.ageAt28##c.`var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3170_new ageAt28 if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3170_new ageAt28 `var', baseoutcome(1) rrr
		est store main
	...
	// Next, go through the remaining categorical variables and code as needed - In all cases will treat lowest category as reference
		...
		if `cats' == 2 {
			...
			mlogit Y3170_new ageAt28 i.`var', baseoutcome(1) rrr
			...
			// Now for interaction model
			mlogit Y3170_new c.ageAt28##i.`var', baseoutcome(1) rrr
...
foreach var of varlist ageAt28 nonWhiteEthnic maritalStatus mobility rural parity education maternalEdu highSocClass income IMD townsendDep housing financeDiffs chLifeEvents_wgt chLifeEvents_total crowding neighPercept partnerAbsence logicMemory-selfEsteem {
	...
	// Next, how we run the analysis will depend on the type of variable - So need to specify whether variable is continuous/binary (as these can be treated the same), or categorical. Will start with cont/binary variables - Although need to analyse 'age' separately first as will be adjusted for in all other models
	if "`var'" == "ageAt28" {
		mlogit Y3155_cat `var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3155_cat if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3155_cat `var', baseoutcome(1) rrr
		est store main
	...
	// Next, analyse the rest of the continuous/binary variables
		...
		mlogit Y3155_cat ageAt28 `var', baseoutcome(1) rrr
		...
		// Now for interaction model
		mlogit Y3155_cat c.ageAt28##c.`var', baseoutcome(1) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3155_cat ageAt28 if `var' != ., baseoutcome(1) rrr
		est store base
		mlogit Y3155_cat ageAt28 `var', baseoutcome(1) rrr
		est store main
	...
	// Next, go through the remaining categorical variables and code as needed - In all cases will treat lowest category as reference
		...
		if `cats' == 2 {
			...
			mlogit Y3155_cat ageAt28 i.`var', baseoutcome(1) rrr
			...
			// Now for interaction model
			mlogit Y3155_cat c.ageAt28##i.`var', baseoutcome(1) rrr
...
foreach var of varlist ageAt28 nonWhiteEthnic maritalStatus mobility rural parity education maternalEdu highSocClass income IMD townsendDep housing financeDiffs chLifeEvents_wgt chLifeEvents_total crowding neighPercept partnerAbsence logicMemory-selfEsteem {
	...
	if "`var'" == "ageAt28" {
		mlogit Y3000 `var', baseoutcome(3) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3000 if `var' != ., baseoutcome(3) rrr
		est store base
		mlogit Y3000 `var', baseoutcome(3) rrr
		est store main
	...
	// Next, analyse the rest of the continuous/binary variables
		...
		mlogit Y3000 ageAt28 `var', baseoutcome(3) rrr
		...
		// Now for interaction model
		mlogit Y3000 c.ageAt28##c.`var', baseoutcome(3) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3000 ageAt28 if `var' != ., baseoutcome(3) rrr
		est store base
		mlogit Y3000 ageAt28 `var', baseoutcome(3) rrr
		est store main
	...
	// Next, go through the remaining categorical variables and code as needed - In all cases will treat lowest category as reference
		...
		if `cats' == 2 {
			...
			mlogit Y3000 ageAt28 i.`var', baseoutcome(3) rrr
			...
			// Now for interaction model
			mlogit Y3000 c.ageAt28##i.`var', baseoutcome(3) rrr
...
foreach var of varlist ageAt28 nonWhiteEthnic maritalStatus mobility rural parity education maternalEdu highSocClass income IMD townsendDep housing financeDiffs chLifeEvents_wgt chLifeEvents_total crowding neighPercept partnerAbsence logicMemory-selfEsteem {
	...
	// Next, how we run the analysis will depend on the type of variable - So need to specify whether variable is continuous/binary (as these can be treated the same), or categorical. Will start with cont/binary variables - Although need to analyse 'ageAt28' separately first as will be adjusted for in all other models
	if "`var'" == "ageAt28" {
		mlogit Y3040_grp `var', baseoutcome(3) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3040_grp if `var' != ., baseoutcome(3) rrr
		est store base
		mlogit Y3040_grp `var', baseoutcome(3) rrr
		est store main
	...
	// Next, analyse the rest of the continuous/binary variables
		...
		mlogit Y3040_grp ageAt28 `var', baseoutcome(3) rrr
		...
		// Now for interaction model
		mlogit Y3040_grp c.ageAt28##c.`var', baseoutcome(3) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3040_grp ageAt28 if `var' != ., baseoutcome(3) rrr
		est store base
		mlogit Y3040_grp ageAt28 `var', baseoutcome(3) rrr
		est store main
	...
	// Next, go through the remaining categorical variables and code as needed - In all cases will treat lowest category as reference
		...
		if `cats' == 2 {
			...
			mlogit Y3040_grp ageAt28 i.`var', baseoutcome(3) rrr
			...
			// Now for interaction model
			mlogit Y3040_grp c.ageAt28##i.`var', baseoutcome(3) rrr
...
** Quick test of whether proportional odds assumption been violated in most basic model (with just age at birth). Ah, it has been violated, and gets worse if add in additional predictors. 
ologit Y3080_OccYr_rev ageAt28, or
brant, detail

ologit Y3080_OccYr_rev ageAt28 i.IMD, or
brant, detail
...
foreach var of varlist ageAt28 nonWhiteEthnic maritalStatus mobility rural parity education maternalEdu highSocClass income IMD townsendDep housing financeDiffs chLifeEvents_wgt chLifeEvents_total crowding neighPercept partnerAbsence logicMemory-selfEsteem {
	...
	// Next, how we run the analysis will depend on the type of variable - So need to specify whether variable is continuous/binary (as these can be treated the same), or categorical. Will start with cont/binary variables - Although need to analyse 'ageAt28' separately first as will be adjusted for in all other models
	if "`var'" == "ageAt28" {
		mlogit Y3080_OccYr_rev `var', baseoutcome(0) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3080_OccYr_rev if `var' != ., baseoutcome(0) rrr
		est store base
		mlogit Y3080_OccYr_rev `var', baseoutcome(0) rrr
		est store main
	...
	// Next, analyse the rest of the continuous/binary variables
		...
		mlogit Y3080_OccYr_rev ageAt28 `var', baseoutcome(0) rrr
		...
		// Now for interaction model
		mlogit Y3080_OccYr_rev c.ageAt28##c.`var', baseoutcome(0) rrr
		...
		// And finally run the likelihood ratio tests
		mlogit Y3080_OccYr_rev ageAt28 if `var' != ., baseoutcome(0) rrr
		est store base
		mlogit Y3080_OccYr_rev ageAt28 `var', baseoutcome(0) rrr
		est store main
	...
	// Next, go through the remaining categorical variables and code as needed - In all cases will treat lowest category as reference
		...
		if `cats' == 2 {
			...
			mlogit Y3080_OccYr_rev ageAt28 i.`var', baseoutcome(0) rrr
			...
			// Now for interaction model
			mlogit Y3080_OccYr_rev c.ageAt28##i.`var', baseoutcome(0) rrr
...
append using ".\G0Mother_Results\mother_belief_2019_results.dta"
replace outcome = "Belief_2019" if outcome == ""
tab outcome, m

append using ".\G0Mother_Results\mother_relig_2019_results.dta"
replace outcome = "Relig_2019" if outcome == ""
tab outcome, m

append using ".\G0Mother_Results\mother_attend_2019_results.dta"
...
outsheet exposure-p_int using ".\G0Mother_Results\IntrinsicCat_coefs.csv" if outcome == "Intrinsic (cat)", comma replace

outsheet exposure-p_int using ".\G0Mother_Results\ExtrinFriends_coefs.csv" if outcome == "Extrinsic - friends", comma replace

outsheet exposure-p_int using ".\G0Mother_Results\ExtrinPray_coefs.csv" if outcome == "Extrinsic - prayer", comma replace

outsheet exposure-p_int using ".\G0Mother_Results\DUREL_coefs.csv" if outcome == "DUREL (cat)", comma replace

outsheet exposure-p_int using ".\G0Mother_Results\belief_2019_coefs.csv" if outcome == "Belief_2019", comma replace

outsheet exposure-p_int using ".\G0Mother_Results\relig_2019_coefs.csv" if outcome == "Relig_2019", comma replace

outsheet exposure-p_int using ".\G0Mother_Results\attend_2019_coefs.csv" if outcome == "Attend_2019", comma replace
...
twoway (scatter level_num coef if outcome == "Belief" & ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.2 0.3 0.5 1 2 5 10 15, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
...
twoway (scatter level_split coef if outcome == "Belief" & exp_level == ///
		...
		xlabel(0.1 0.2 0.3 0.5 1 2 3 5 8, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
...
twoway (scatter level_num coef if outcome == "Belief" & exposure == ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.8 1 1.5 2 3, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
...
twoway (scatter level_num coef if outcome == "Belief" & exposure == ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.6 0.8 1 1.5 2 2.5, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
...
twoway (scatter level_split coef if outcome == "Belief" & exp_level == ///
		...
		xline(1, lcol(black) lpattern(shortdash) lwidth(thin)) xscale(log) ///
		xlabel(0.3 0.5 1 2 3, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
...
twoway (scatter level_split coef if outcome == "Belief" & exp_level == ///
		...
		xline(1, lcol(black) lpattern(shortdash) lwidth(thin)) xscale(log) ///
		xlabel(0.3 0.5 1 2 3 4, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
...
twoway (scatter level_num coef_int if outcome == "Belief" & exposure == ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.9 0.95 1 1.05 1.1 1.14, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
...
twoway (scatter level_split coef_int if outcome == "Belief" & exp_level == ///
		...
		xline(1, lcol(black) lpattern(shortdash) lwidth(thin)) xscale(log) ///
		xlabel(0.8 1 1.2 1.4 1.6, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8 10 11 12 14 15 16 18 19 20 22 23 24 25 ///
		, valuelabel labsize(vsmall) ///
		angle(0)) legend(order(1 "Rented" 15 "Council/HA" 29 "Other") ///
			rows(1)) ///
...
keep if exposure == "highSocClass" | exposure == "IPSM_total" | exposure == "LoC_external" | exposure == "ageAt28" | exposure == "ageAtBirth" | exposure == "education" | exposure == "income" | exposure == "intel_factor" | exposure == "maritalStatus" | exposure == "nonWhiteEthnic"
...
** Start with age plot
replace exposure = "Age" if exposure == "ageAtBirth" | exposure == "ageAt28"
tab exposure
...
twoway (scatter level_split coef if time == "preg" & exposure == "Age", ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.96 1 1.04 1.08 1.12, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8, valuelabel labsize(small) ///
		angle(0)) legend(order(1 "Pregnancy data" 3 "2019 data")) ///
		name(age, replace)
		
graph export ".\G0Mother_Results\Age_pregVs2019.pdf", replace
...
twoway (scatter level_split coef if time == "preg" & exposure == "nonWhiteEthnic", ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.5 0.7 1 2 3 5 10 15, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8, valuelabel labsize(small) ///
		angle(0)) legend(order(1 "Pregnancy data" 3 "2019 data")) ///
		name(ethnic, replace)
		
graph export ".\G0Mother_Results\Ethnic_pregVs2019.pdf", replace
...
twoway (scatter level_split coef if time == "preg" & exp_level == ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.5 0.7 1 2 3 5 8, labsize(small)) ///
...
graph export ".\G0Mother_Results\Marital_pregVs2019.pdf", replace
...
twoway (scatter level_split coef if time == "preg" & exp_level == ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.4 0.5 0.7 1 2 3 5 10 15, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8, valuelabel labsize(vsmall) ///
...
graph export ".\G0Mother_Results\Education_pregVs2019.pdf", replace
...
twoway (scatter level_split coef if time == "preg" & exposure == "highSocClass", ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.6 0.8 1 1.5 2 3, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8, valuelabel labsize(small) ///
		angle(0)) legend(order(1 "Pregnancy data" 3 "2019 data")) ///
		name(socClass, replace)
		
graph export ".\G0Mother_Results\socClass_pregVs2019.pdf", replace
...
graph export ".\G0Mother_Results\income_pregVs2019.pdf", replace


** Cognitive ability
sum lci uci if level_split < . & exposure == "intel_factor"

twoway (scatter level_split coef if time == "preg" & exposure == "intel_factor", ///
			col(black) msize(small) msym(D)) ///
		(rspike lci uci level_split if time == "preg" & exposure == "intel_factor", ///
			horizontal lwidth(thin) col(black)) ///
		(scatter level_split coef if time == "2019" & exposure == "intel_factor", ///
			col(red) msize(small) msym(D)) ///
		(rspike lci uci level_split if time == "2019" & exposure == "intel_factor", ///
			horizontal lwidth(thin) col(red)), ///
		yscale(reverse)	ytitle("") ///
		xtitle("Relative risk ratio") ///
		title("Cog. ability and RSBB - Pregnancy vs 2019", size(medium)) ///
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(0.7 0.8 0.9 1 1.1 1.2 1.3, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8, valuelabel labsize(small) ///
		angle(0)) legend(order(1 "Pregnancy data" 3 "2019 data")) ///
...
graph export ".\G0Mother_Results\cog_pregVs2019.pdf", replace
...
twoway (scatter level_split coef if time == "preg" & exposure == "IPSM_total", ///
		...
		xline(1, lcol(black) lpattern(shortdash)) xscale(log) ///
		xlabel(1 1.01 1.02, labsize(small)) ///
		ylabel(0 1 3 4 6 7 8, valuelabel labsize(small) ///
		angle(0)) legend(order(1 "Pregnancy data" 3 "2019 data")) ///
		name(ipsm, replace)
		
graph export ".\G0Mother_Results\ipsm_pregVs2019.pdf", replace
...
graph export ".\G0Mother_Results\loc_pregVs2019.pdf", replace
